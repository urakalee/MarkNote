...
public class CategoryViewModel extends BaseViewModel<Category> {
    ...
    public String getEmptySubTitle(ItemStatus status) {
        ...
        return PalmApp.getContext().getString(
                status == ItemStatus.NORMAL ? R.string.tags_list_empty_sub_normal :
                        status == ItemStatus.TRASHED ? R.string.tags_list_empty_sub_trashed :
                                status == ItemStatus.ARCHIVED ? R.string.tags_list_empty_sub_archived :
